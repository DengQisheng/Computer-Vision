...
def train(train_loader, valid_loader, network, criterion, optimizer, scheduler, num_epochs, log_show, log_root, device, depth):
    ...
    with open(f'{log_root}/ResNet{depth}_train_cutmix.log', 'w') as logging:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for patch, label in train_loader:
                ...
                rand_index = torch.randperm(patch.size()[0]).to(device)
                label_a = label
                label_b = label[rand_index]
                bbx1, bby1, bbx2, bby2 = cutmix_box(patch.size(), lam)
                patch[:, :, bbx1:bbx2, bby1:bby2] = patch[rand_index, :, bbx1:bbx2, bby1:bby2]
                lam = 1 - ((bbx2 - bbx1) * (bby2 - bby1) / (patch.size()[-1] * patch.size()[-2]))
                batch += 1
                # Forward
                prediction = network(patch)
                loss = lam * criterion(prediction, label_a) + (1 - lam) * criterion(prediction, label_b)
                # Backprop
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                ...
                train_correct += lam * predicted.eq(label_a.data).cpu().sum().float() + (1 - lam) * predicted.eq(label_b.data).cpu().sum().float()
...
def save_model(root, model, depth):
    torch.save(model.state_dict(), f'{root}/ResNet{depth}_cutmix.pth')
    print(f'Model ResNet{depth}_cutmix.pth saved!')
...
def visual(root, history, num_epochs, depth):
    ...
    plt.legend()
    plt.savefig(f'{root}/ResNet{depth}_loss_curve_cutmix.png')
    plt.close('all')
    print(f'ResNet{depth}_loss_curve_cutmix.png saved!')
    ...
    plt.xticks(list(range(0, num_epochs + 1, 25)))
    plt.legend()
    plt.savefig(f'{root}/ResNet{depth}_acc_curve_cutmix.png')
    plt.close('all')
    print(f'ResNet{depth}_acc_curve_cutmix.png saved!')
