...
def train(train_loader, valid_loader, network, criterion, optimizer, scheduler, num_epochs, log_show, log_root, device, depth):
    ...
    with open(f'{log_root}/ResNet{depth}_train_mixup.log', 'w') as logging:
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for patch, label in train_loader:
                ...
                patch, label_a, label_b, lam = mixup_data(patch, label, device=device)
                batch += 1
                # Forward
                prediction = network(patch)
                loss = mixup_criterion(criterion, prediction, label_a, label_b, lam)
                # Backprop
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
def save_model(root, model, depth):
    torch.save(model.state_dict(), f'{root}/ResNet{depth}_mixup.pth')
    print(f'Model ResNet{depth}_mixup.pth saved!')
...
def visual(root, history, num_epochs, depth):
    ...
    plt.legend()
    plt.savefig(f'{root}/ResNet{depth}_loss_curve_mixup.png')
    plt.close('all')
    print(f'ResNet{depth}_loss_curve_mixup.png saved!')
    ...
    plt.xticks(list(range(0, num_epochs + 1, 25)))
    plt.legend()
    plt.savefig(f'{root}/ResNet{depth}_acc_curve_mixup.png')
    plt.close('all')
    print(f'ResNet{depth}_acc_curve_mixup.png saved!')
