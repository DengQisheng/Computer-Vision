...
def train(train_loader, valid_loader, network, criterion, optimizer, scheduler, num_epochs, log_show, log_root, device, depth):
    ...
    with open(f'{log_root}/ResNet{depth}_train_baseline.log', 'w') as logging:
...
def save_model(root, model, depth):
    torch.save(model.state_dict(), f'{root}/ResNet{depth}_baseline.pth')
    print(f'Model ResNet{depth}_baseline.pth saved!')
...
def visual(root, history, num_epochs, depth):
    ...
    plt.legend()
    plt.savefig(f'{root}/ResNet{depth}_loss_curve_baseline.png')
    plt.close('all')
    print(f'ResNet{depth}_loss_curve_baseline.png saved!')
    ...
    plt.xticks(list(range(0, num_epochs + 1, 25)))
    plt.legend()
    plt.savefig(f'{root}/ResNet{depth}_acc_curve_baseline.png')
    plt.close('all')
    print(f'ResNet{depth}_acc_curve_baseline.png saved!')
